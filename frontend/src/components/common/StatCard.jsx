...
export const StatCard = ({
  ...
  return (
    <div
      ...
      className="flex flex-col gap-4 rounded-xl bg-slate-800/40 p-5 border border-slate-700/50"
    >
      <div className="flex items-start justify-between gap-3">
        <span className="text-sm font-medium text-slate-400 leading-snug">{title}</span>
        {Icon && (
          <div
      ...
      <div>
        <div className="text-[1.85rem] font-semibold tracking-tight text-slate-100 tabular-nums leading-none">
          {value}
        </div>
        {subtitle && (
          <p className="mt-2 text-sm text-slate-400 leading-snug">{subtitle}</p>
        )}
      </div>
